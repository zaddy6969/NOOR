  ...
  return (
    <section className="shop-working-area">
      <div className="shop-catalogue">
        <div className="directory-toolbar shop-toolbar"><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search Quran, prayer mat, Ihram…" aria-label="Search shop catalogue"/><div role="group" aria-label="Shop categories">{categories.map((item) => <button className={filter === item ? "active" : ""} type="button" onClick={() => setFilter(item)} key={item}>{item}</button>)}</div><span>{visible.length} items</span></div>
      ...
      <aside className="shop-request-card"><span>YOUR REQUEST LIST</span><strong>{selected.length} {selected.length === 1 ? "item" : "items"}</strong>{selected.length ? <div>{selected.map((item) => <button type="button" onClick={() => toggle(item.id)} key={item.id}><span><b>{item.name}</b><small>{item.category}</small></span><i aria-label={`Remove ${item.name}`}>×</i></button>)}</div> : <p>Add useful items from any category. Your list stays privately on this device.</p>}<footer><b>Ordering is not active yet</b><p>Prices, stock, delivery and payment will appear only when verified sellers are connected.</p></footer></aside>
    </section>
  );
}
