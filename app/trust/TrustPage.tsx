...
export type TrustSection = { id?: string; title: string; paragraphs?: string[]; points?: string[] };
...
export default function TrustPage({ title, kicker, intro, sections }: { title: string; kicker: string; intro: string; sections: TrustSection[] }) {
  return (
    <main className="trust-page">
      ...
      <section className="trust-sections">{sections.map((section) => <article id={section.id} key={section.title}><h2>{section.title}</h2>{section.paragraphs?.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.points ? <ul>{section.points.map((point) => <li key={point}>{point}</li>)}</ul> : null}</article>)}</section>
      <SiteFooter />
    </main>
  );
}
